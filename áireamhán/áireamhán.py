...
_quote, _if, _define, _lambda, _begin, _set, = map(
    Sym, "athfhriotal   má   sainigh   lambda   tosaigh  cuir!".split()
)
    ...
    self.update(
        {
            ...
            "liosta?": lambda x: isa(x, list),
            "folamh?": lambda x: x == [],
            "adamh?": lambda x: not ((isa(x, list)) or (x == None)),
            "boole?": lambda x: isa(x, bool),
            "scag": lambda f, x: list(filter(f, x)),
            "cuir-le": lambda proc, l: proc(*l),
            "mapáil": lambda p, x: list(map(p, x)),
            "lódáil": lambda fn: load(fn),
            "léigh": lambda f: f.read(),
            "léigh-línte": lambda f: [l.strip("\n") for l in f.readlines()],
            "oscail_comhad_ionchuir": open,
            "dún_comhad_ionchuir": lambda p: p.file.close(),
            "oscail_comhad_aschur": lambda f: open(f, "w"),
            "dún_comhad_aschur": lambda p: p.close(),
            "dac?": lambda x: x is eof_object,
            "luacháil": lambda x: evaluate(x),
